      ...
      <form className="flex-grow text-slate-400 justify-center items-center mt-3">
        <ul className="flex flex-wrap justify-center items-end  box-border h-64">
          {Object.keys(regions).map((region) => {
            ...
            return (
              <li
                className={`w-2/6 cursor-pointer mx-auto flex my-1 rounded-lg ${
                  regionValue == regionKey ? "bg-secondary" : ""
                }`}
                key={regionKey}
              >
                <input
                ...
                />
                <label
                  className="cursor-pointer flex-grow text-center font-semibold"
                  htmlFor={regionKey}
                >
        ...
        <div className="flex justify-center mt-6">
          <button
            className="bg-zinc-300 dark:bg-secondary focus:bg-slate-300 dark:focus:bg-slate-600 font-bold h-5/6 my-auto mr-1 px-6 py-1 rounded-md hover:outline focus:outline focus:outline-2"
            type="button"
            onClick={() => closeModal(regionValue)}
          >
            Submit
          </button>
        </div>
